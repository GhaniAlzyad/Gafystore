...
from App.src.schema import OrderdetailRequestSchema, OrderdetailResponseSchema, OrderdetailUpdateSchema, OrderDetailReport
...
@api.post("/", response_model=OrderdetailResponseSchema)
async def create_order(order: OrderdetailRequestSchema, token:str = Header(None)):
    auth = await token_validator(token)
    if auth:
        return await OrderLogic.add_order(order)


@api.get("/{id}", response_model=OrderdetailResponseSchema)
...
@api.get("/customer/{id}", response_model=List[OrderdetailResponseSchema])
async def get_by_user_id(id: str):
    orders = await Order.get_all_by_user_id(id)
    return orders


@api.put('/{id}', response_model=OrderdetailResponseSchema)
async def update(id: int, order: OrderdetailUpdateSchema):
...
async def get_user(id: int):
    order = await Order.get(id)
    result = await OrderdetailResponseSchema.from_model(order)
    return result
